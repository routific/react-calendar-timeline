...
export function getMinUnit(zoom, width, timeSteps) {
  ...
  Object.keys(timeDividers).some(unit => {
    ...
    nextTimeSpanInUnitContext /= timeDividers[unit];
...
export function getGroupWithItemDimensions(
  ...
  const itemDimensions = groupWithItems.items.map(item => getItemDimensions({
    item,
    keys,
    canvasTimeStart,
    canvasTimeEnd,
 ...
 * @param {*} keys
 */
export const getGroupsCache = memoize((groups, keys, method) => groups.reduce((acc, group) => {
